...
// Routes
app.get('/', (req, res) => {
    res.send('Rangoons Android App - Connected to www.rangoons.live');
});
...
app.get('/api/health', (req, res) => {
    res.json({ 
        status: 'OK',
        app: 'Rangoons Android',
        timestamp: new Date().toISOString()
    });
});

app.listen(port, '0.0.0.0', () => {
    console.log(`📱 Android app listening on port ${port}`);
    console.log(`🌐 Server: http://0.0.0.0:${port}`);
    console.log(`🏥 Health: http://0.0.0.0:${port}/health`);
    console.log(`📊 Status: http://0.0.0.0:${port}/status`);
});

console.log('🚀 Rangoons Android app starting...');
